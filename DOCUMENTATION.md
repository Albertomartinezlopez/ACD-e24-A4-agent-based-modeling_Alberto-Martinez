...
Ensure the points are valid 3D points and that there are at least two valid points.
Proportional Z Translation:

Calculate the Z movement for each point based on its position in the list (first point moves the most, last one the least).
Apply Movement:

Move each point by the corresponding Z distance and store the result.
Return Results:

Output the list of moved points.
...
Attributes:
tool_radius, tool_height, tool
Methods:
move_to(): Moves the tool to a specified point.
generate_tool(): Generates and returns the milling tool (cylinder) at a specific position.
Main Milling Process:

Steps:
Initialize the agent and toolpath.
Loop through each toolpath point and move the agent.
Perform the boolean difference operation to subtract the milling tool from the block.
Output the final geometry after all milling steps.
---
